...
#     with tf.variable_scope('MESSAGE_ENCODER') as message_scope:
#         if lstm_step > 0:
#             message_scope.reuse_variables()
#         tf_message_input = tf.nn.embedding_lookup(tf_learned_embeddings, tf_message[:, lstm_step])
...
# response_state = response_lstm.zero_state(tf_batch_size, tf.float32)
# response_outputs = []
# for lstm_step in range(MAX_MESSAGE_LENGTH):
#     with tf.variable_scope('RESPONSE_DECODER') as response_scope:
#         if lstm_step > 0:
...
# tf_response_outputs = tf.stack(response_outputs, axis=1)
...
probability_layer = keras.layers.core.Dense(RNN_HIDDEN_DIM)
