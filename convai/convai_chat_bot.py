...
import config
from chat_bots import chat_model_func, latent_chat_func
...
with tf.Graph().as_default() as latent_chat_model_graph:
    ...
    lcm = latent_chat_func.LatentChatModel(len(chat_vocab_dict), latent_chat_func.LEARNING_RATE,
                                           options.save_dir,
...
while True:
    ...
    np_question = chat_model_func.construct_numpy_from_messages([tk_message.split()], vocab_dict, config.MAX_QUESTION_WORDS)
    np_context = chat_model_func.construct_numpy_from_messages([tk_context.split()], vocab_dict, config.MAX_CONTEXT_WORDS)
