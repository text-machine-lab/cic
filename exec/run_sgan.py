...
def main(code_size, regen_latent_ae, max_number_of_sentences, num_gen_layers, num_dsc_layers,
    ...
    if regen_latent_ae:
        ...
        encoder = AutoEncoder(len(ds.vocab), save_dir=cic.paths.GM_AE_SAVE_DIR,
    ...
    # Train SentenceGAN.
    if num_epochs > 0:
        gan.train(merge_dataset, params={'gen_lr': gen_learning_rate,
    ...
    decoder = AutoEncoder(len(ds.vocab), save_dir=cic.paths.GM_AE_SAVE_DIR,
    ...
    if save_gen_sents:
        pickle.dump(generated_sentences, open(os.path.join(cic.paths.DATA_DIR, 'gan_messages.pkl'), 'wb'))
